...
from dexumi.hand_sdk.inspire.hand_api_cls import ExoInspireSDK, InspireSDK
...
DEFAULT_MODEL_DIR = os.path.abspath(
    os.path.join(SCRIPT_DIR, "../../data_local/visual_regression_xhand")
)
...
def main(enable_encoder, joint_index, model_dir):
    ...
    time.sleep(2)
    hand = ExoXhandSDK(port="/dev/ttyUSB0")
    ...
    if hand.connect():
        ...
        if joint_index == 0:
            target_motor_value = np.linspace(15, 110, 15).astype(np.int32)
        elif joint_index == 1:
            target_motor_value = np.linspace(0, 90, 10).astype(np.int32)
        elif joint_index == 2:
            target_motor_value = np.linspace(0, 105, 10).astype(np.int32)
        elif joint_index == 3:
            target_motor_value = np.linspace(-10, 10, 10).astype(np.int32)
        else:
            target_motor_value = np.linspace(5, 110, 10).astype(np.int32)

        target_motor_value = target_motor_value / 180 * np.pi
        ...
        while True:
            with FrameRateContext(
                ...
                if exec:
                    ...
                    desired_joint_angles[[5, 7, 9, 11]] = 5 / 180 * np.pi
                    if joint_index == 1 or joint_index == 2:
                        desired_joint_angles[0] = 75 / 180 * np.pi
                    # if joint_index == 0:
                    #     desired_joint_angles[1] = 40 / 180 * np.pi
